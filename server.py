...
@app.route(
    "/collections/get_route_data/<route_id>/route_geometry.json"
)  # make into queue job, some job_id
@login_required
def get_existing_route_data(route_id):
    """Given route id, return route json"""

    route_geometry_obj = RouteGeometry.query.filter(
        (RouteGeometry.route_id == route_id)
    ).first()
...
@app.route(
    "/collections/get_route_data/<route_id>/edges_geometry.json"
)  # make into queue job, some job_id
@login_required
def get_existing_edges_data(route_id):
    """Given route id, return edges geometry json"""

    edges_geometry_obj = EdgesGeometry.query.filter(
        (EdgesGeometry.route_id == route_id)
    ).first()
...
@app.route(
    "/collections/get_route_data/<route_id>/nodes_geometry.json"
)  # make into queue job, some job_id
@login_required
def get_existing_nodes_data(route_id):
    """Given route id, return edges geometry json"""

    nodes_geometry_obj = NodesGeometry.query.filter(
        (NodesGeometry.route_id == route_id)
    ).first()
...
@app.route(
    "/collections/get_route_data/<route_id>/animate_point_geometry.json"
)  # make into queue job, some job_id
@login_required
        ...
        "features": [
            {
                ...
                "geometry": {
                    "type": "Point",
                    "coordinates": route_geojson["features"][0]["geometry"][
                        "coordinates"
                    ][0],
                },
            }
...
def get_bounds_array_for_collection(collection_id):
    ...
    all_routes_in_collection = Route.query.filter(
...
@app.route(
    "/collections/get_collection_data/<collection_id>/all_routes.json"
)  # make into queue job, some job_id
@login_required
...
def save_route():
    ...
    collection = Collection.query.filter(
        (Collection.collection_name == desination_collection_name)
    ...
    if collection == None:
        ...
        added_collection = Collection.query.filter(
            Collection.collection_name == desination_collection_name
        ).first()
        ...
        route = Route.query.filter(
            (Route.route_name == new_route_name)
            & (Route.collection_id == collection.collection_id)
        ).first()

        # if route already exists return success as false

        if route != None:
            return jsonify({"route_name": new_route_name, "success": "false"})
    ...
    # add bbox
    new_box_geom = BboxGeometry(route_id=route_id, bbox_geometry=bbox_data)
    # add to the session and commit
    db.session.add(new_box_geom)
    db.session.commit()

    # get route id for the route that was just added
    new_route_geom = RouteGeometry(
        route_id=route_id, route_geometry=route_line_data, route_length=route_length
    )
    ...
    new_nodes_geom = NodesGeometry(route_id=route_id, nodes_geometry=nodes_data)

    db.session.add(new_nodes_geom)
    db.session.commit()

    new_edges_geom = EdgesGeometry(route_id=route_id, edges_geometry=edges_data)

    db.session.add(new_edges_geom)
    db.session.commit()

    return jsonify({"route_name": new_route_name, "success": "true"})
