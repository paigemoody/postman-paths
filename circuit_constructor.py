...
def choose_edge_to_traverse(current_node, possible_next_edges, current_bridges):
    ...
    for possible_next_edge in possible_next_edges:
        ...
        if (
            possible_next_edge in current_bridges
            or possible_next_edge[::-1] in current_bridges
        ):
...
def make_node_geojson(graph_instance):
    ...
    for node in graph_instance.node_visit_order:
        ...
        point_coords_list = [
            list(coord_tuple) for coord_tuple in list(shape(point_shapely).coords)
        ]
        ...
        node_feature = {
            ...
            "geometry": {"type": "Point", "coordinates": point_coords_list[0]},
        }
...
def make_edge_geojson(graph_instance):
    ...
    for edge in list(edge_visit_dict.keys()):
        ...
        line_coords_list = [
            list(coord_tuple) for coord_tuple in list(shape(line_shapely).coords)
        ]
        ...
        edge_feature = {
                ...
                "nodes": {
                    edge[0]: [
                        ...
                        for coord_tuple in (
                            list(
                                shape(
                                    graph_instance.nodes_dict[edge[0]]["geometry"]
                                ).coords
                            )
                        )
                    ],
                    edge[1]: [
                        ...
                        for coord_tuple in (
                            list(
                                shape(
                                    graph_instance.nodes_dict[edge[1]]["geometry"]
                                ).coords
                            )
                        )
                    ],
            ...
            "geometry": {"type": "LineString", "coordinates": line_coords_list},
        }
...
def make_route_geojson(graph_instance):
    ...
    for edge in edge_visit_order:
        ...
        first_node_coords = [
            list(coord_tuple) for coord_tuple in list(shape(first_node_geometry).coords)
        ]
        ...
        edge_geometry_coords = [
            list(coord_tuple) for coord_tuple in list(shape(edge_geometry).coords)
        ]
    ...
    bbox_geom_str = str(
        {
            ...
            "features": [
                {
                    "id": "2a01221a7db21efadd487199ba4d89d9",
                    "type": "Feature",
                    "properties": {},
                    "geometry": {
                        "coordinates": [
                            [
                                [-122.40017025263401, 37.76001428340129],
                                [-122.40238039286139, 37.75982767848936],
                                [-122.4020048835992, 37.756841935919326],
                                [-122.39958016664835, 37.75696068934555],
                                [-122.40017025263401, 37.76001428340129],
                            ]
                        ],
                        "type": "Polygon",
